...
AUTHORS_FILE = (
    rf"{ONEDRIVE_DIR}\authors.xlsx"  # manually edited xlsx, lives in OneDrive
)
OUTPUT_DIR = "out"
...
PUBLISHER_GROUPS = [
    ...
    (
        ...
        r"|Harper Perennial|Harper & Row|Fourth Estate",
        "HarperCollins",
    ),
    (r"\bOrbit\b", "Orbit"),
    ...
    (
        ...
        r"|Del Rey|\bVintage\b|\bKnopf\b|Anchor Books|Doubleday|\bSpectra\b",
        "Random House",
    ),
...
def small_multiples_annual(pivot_df, title, filename, extra_panels=None):
    ...
    fig, axes = plt.subplots(
        nrows, ncols, figsize=(3.2 * ncols, 2.8 * nrows), sharex=True, squeeze=False
    )
...
tb["author_1_name"] = (
    tb["author_1_name"].astype(str).str.strip().str.replace(r"\s+", " ", regex=True)
)
...
tb.loc[missing_start, "started_at"] = tb.loc[missing_start, "read_at"] - pd.Timedelta(
    weeks=2
)
...
if has_duration.empty:
    ...
    plt.savefig(
        f"{OUTPUT_DIR}/{READER_NAME}_bookWaterfall_allTime.pdf", bbox_inches="tight"
    )
...
if has_data(tb["publication_year"]):
    ...
    tb.publication_year[tb.publication_year >= 1900].hist(
        bins=40, ax=ax_ins, color=SINGLE_COLOR, alpha=0.8
    )
    ax_ins.grid(False)
    ...
    if not lag.empty:
        ...
        med_lag = (
            lag_df.loc[lag_df["lag"].between(0, 500)]
            .groupby(lag_df["read_at"].dt.year)["lag"]
            .median()
        )
        fig, ax = plt.subplots()
...
if has_data(tb["num_pages"]):
    print(
        f"Median pages: {int(tb.num_pages.median())},  mean: {int(tb.num_pages.mean())}"
    )
    fig, ax = plt.subplots()
...
if not os.path.exists(AUTHORS_FILE):
    ...
    print(
        "Create authors.xlsx with columns: name, gender, ethnicity, DeadorAlive, Nationality, LGBTQI_Authors"
    )
else:
    ...
    authors_df.compound_diversity.value_counts().plot(
        kind="bar", figsize=(12, 6), rot=0, color=SINGLE_COLOR
    )
    ...
    authors_df.compound_sexuality.value_counts().plot(
        kind="bar", figsize=(10, 6), rot=0, color=SINGLE_COLOR
    )
    ...
    authors_df.compound_genderalive.value_counts().plot(
        kind="bar", figsize=(10, 6), rot=0, color=SINGLE_COLOR
    )
    ...
    if all_df.empty:
        ...
        if not counts_2d.empty:
            ...
            fig, ax = plt.subplots(
                figsize=(max(5, len(ethnicities) * 1.2), max(3, len(genders) * 0.9))
            )
            for _, row in counts_2d.iterrows():
                xi = ethnicities.index(row["ethnicity"])
                yi = genders.index(row["gender"])
                ax.scatter(
                    xi, yi, s=row["n"] * 20, color=SINGLE_COLOR, alpha=0.7, zorder=3
                )
                ax.text(
                    xi,
                    yi,
        ...
        if not rated_all.empty:
            ...
            for ax, col, title in zip(
                ...
                ax.hlines(
                    means.index, 0, means.values, color="lightgray", linewidth=1.5
                )
                ax.scatter(
                    means.values, means.index, s=70, color=SINGLE_COLOR, zorder=3
                )
        ...
        if not dated_authors.empty:
            short_year_index(
                dated_authors.groupby(["Year", "compound_diversity"])
                .size()
            ...
            short_year_index(
                dated_authors.groupby(["Year", "compound_genderalive"])
                .size()
                .unstack(fill_value=0)
            ).plot(kind="bar", stacked=True, rot=0)
            ...
            if "ficOrNonFic" in dated_authors.columns and has_data(
                dated_authors["ficOrNonFic"]
            ):
                short_year_index(
                    dated_authors.groupby(["Year", "ficOrNonFic"])
                    .size()
                    .unstack(fill_value=0)
                ).plot(kind="bar", stacked=True, rot=0)
            ...
            small_multiples_annual(
                ...
                .size()
                .unstack(fill_value=0),
                "Gender and life status by year",
                "annualGenderAlive_sm",
            )
            if "ficOrNonFic" in dated_authors.columns and has_data(
                dated_authors["ficOrNonFic"]
            ):
                small_multiples_annual(
                    ...
                    .size()
                    .unstack(fill_value=0),
                    "Fiction vs non-fiction by year",
                    "annualFicNonfic_sm",
                )
        ...
        all_df.groupby(["compound_diversity", "DeadorAlive"]).size().unstack(
            fill_value=0
        ...
        if "ficOrNonFic" in all_df.columns and has_data(all_df["ficOrNonFic"]):
            fig, ax = plt.subplots(figsize=(12, 6))
            all_df.groupby(["compound_diversity", "ficOrNonFic"]).size().unstack(
                fill_value=0
            ).plot(kind="bar", stacked=True, rot=0, ax=ax)
        ...
        if has_data(all_df["num_pages"]):
            s_pub = (
                all_df.dropna(subset=["publication_year"])
                .groupby("publication_year")["num_pages"]
                .sum()
                .sort_index()
            )
            fig, ax = plt.subplots()
            ...
            ax_ins2 = ax.inset_axes(
                [-500, ymax * 0.35, 2000, ymax * 0.58], transform=ax.transData
            )
            s_1900 = s_pub[s_pub.index >= 1900]
            ax_ins2.bar(
                s_1900.index.astype(int),
                s_1900.values,
                width=0.8,
                color=SINGLE_COLOR,
                alpha=0.8,
            ...
            fig, ax = plt.subplots()
            ax.plot(
                s_read.index.astype(int), s_read.values, "o-", lw=2, color=SINGLE_COLOR
            )
            ...
            if "ficOrNonFic" in all_df.columns and has_data(all_df["ficOrNonFic"]):
                all_df.groupby(["compound_diversity", "ficOrNonFic"])[
                    "num_pages"
                ].sum().unstack(fill_value=0).plot(
                    kind="bar", stacked=True, rot=0, figsize=(12, 6)
                )
